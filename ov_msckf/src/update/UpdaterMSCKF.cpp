...
void UpdaterMSCKF::update(std::shared_ptr<State> state, std::vector<std::shared_ptr<Feature>> &feature_vec) {
  ...
  // By default this is computed from the persistent SLAM feature set inside
  // UpdaterSLAM (matches the paper). When cbf_use_slam_features == false we
  // instead compute it here from the MSCKF feature set.
  // ============================================================================
  if (!_options.cbf_use_slam_features) {
    UpdaterHelper::CbfOutput raw = UpdaterHelper::compute_cbf_metrics(state, feature_vec, clones_cam);
    _cbf_output = UpdaterHelper::smooth_cbf_metrics(raw, _cbf_ema);
    if (_cbf_output.valid) {
      PRINT_INFO(CYAN "[CBF-MSCKF]: feats=%d | logdet=%.4f | drift=%.6f | g=[%.6f, %.6f, %.6f]\n" RESET, _cbf_output.num_features,
                 _cbf_output.mean_logdet, _cbf_output.drift, _cbf_output.g_vec(0), _cbf_output.g_vec(1), _cbf_output.g_vec(2));
    }
  } else {
    _cbf_output = UpdaterHelper::CbfOutput(); // computed in UpdaterSLAM instead
  }
